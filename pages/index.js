...
import { getBlobUrl } from '../azureStorage';
...
export default function Home() {
  ...
  async function fetchPosts() {
    ...
    // Fetch images from Azure Blob Storage for posts that contain a cover image
    const postsWithImages = await Promise.all(items.map(async post => {
      if (post.coverImage) {
        post.coverImage = await getBlobUrl(post.coverImage);
      }
      return post;
    }));
